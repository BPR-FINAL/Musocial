# 🚀 Musocial - Sosyal Medya Sitesi
<div align="center">

![PHP](https://img.shields.io/badge/PHP-8.0+-777BB4?style=for-the-badge&logo=php&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-8.0+-4479A1?style=for-the-badge&logo=mysql&logoColor=white)
![Bootstrap](https://img.shields.io/badge/Bootstrap-5.3-7952B3?style=for-the-badge&logo=bootstrap&logoColor=white)
![Security](https://img.shields.io/badge/Security-A+-00C851?style=for-the-badge&logo=security&logoColor=white)
![License](https://img.shields.io/badge/License-MIT-yellow?style=for-the-badge)

**Güvenlik Önlemleri Alınmış Sosyal Medya Sitesi**

[Özellikler](#-özellikler) • [Kurulum](#-kurulum) • [Güvenlik](#-güvenlik-özellikleri) • [Kullanım](#-kullanım)

</div>

---

## 👥 Proje Ekibi

Bu proje **Bartın(Bartın MYO) Üniversitesi - İnternet Tabanlı Programlama (BPR201)** dersi kapsamında geliştirilmiştir.

| Öğrenci Adı | Öğrenci No | Rol | Katkı |
|-------------|------------|-----|-------|
| **Enes Yasin TOPAL** | **24015221051** | Full-Stack Developer & Security Lead | Backend, Security, Database, Testing|
| **Musa YILMAZ** | **24015221061** | Full-Stack Developer  | Frontend,Backend, Design, Testing,Database|

---
**Hoca:** Dr. Öğr. Serkan Aksu  - İnternet Tabanlı Programlama Dersi  
**Dönem:** 2025-2026 Güz Dönemi  
**Teslim Tarihi:** 18 Aralık 2025  
**GitHub:** [@BPRFINAL](https://github.com/BPR-FINAL/Musocial)

---

## 🌟 Genel Bakış

**Musocial**, modern web teknolojileri ile geliştirilmiş kapsamlı bir Sosyal medyadır. Proje, kullanıcı dostu arayüzü, gelişmiş yönetim panelidir.



---

## 📦 Kurulum

### Sistem Gereksinimleri

```bash
✅ PHP >= 8.0 (önerilen: 8.2)
✅ MySQL >= 8.0 veya MariaDB >= 10.5
✅ Apache/Nginx web server
✅ mod_rewrite enabled (Apache)
✅ PHP Extensions:
   - PDO
   - PDO_MySQL
   - mbstring
   - openssl
   - session
```

#### 6️⃣ Varsayılan Hesap

**Admin Hesabı:**
```
Kullanıcı: admin1
Şifre: admin123456
```
---




**Bartın Üniversitesi (BARTIN MYO) - İnternet Tabanlı Programlama (BPR201)**

</div>
